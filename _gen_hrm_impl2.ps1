...
public class HrmSalaryServiceImpl extends ServiceImpl<HrmSalaryMapper, HrmSalary> implements IHrmSalaryService {
    ...
    public void calculate(String salaryMonth) {
        ...
        for (HrmSalary salary : list) {
            // 实发 = 基本工资 + 绩效奖金 + 加班费 + 津贴 - 扣款 - 社保 - 公积金 - 个税
    ...
    public Map<String, Object> paySlip(Long id) {
        ...
        slip.put("income", salary.getBaseSalary()
        ...
        slip.put("deductions", (salary.getDeduction() != null ? salary.getDeduction() : BigDecimal.ZERO)
...
"@ | Set-Content "$base\service\impl\HrmSalaryServiceImpl.java" -Encoding UTF8
...
"@ | Set-Content "$base\service\impl\HrmPerformanceServiceImpl.java" -Encoding UTF8
...
public class HrmTrainingServiceImpl extends ServiceImpl<HrmTrainingMapper, HrmTraining> implements IHrmTrainingService {
    ...
    public void enroll(Long trainingId, Long employeeId) {
        // 简化实现：仅更新参与人数逻辑（实际需要关联表）
        HrmTraining training = trainingMapper.selectById(trainingId);
        if (training == null) {
            throw new com.zhehang.erp.common.core.exception.BusinessException("培训不存在");
        }
        if (training.getStatus() != 1) {
            throw new com.zhehang.erp.common.core.exception.BusinessException("该培训未开放报名");
        }
    }
}
"@ | Set-Content "$base\service\impl\HrmTrainingServiceImpl.java" -Encoding UTF8

Write-Host "Service impls part 2 generated!"
